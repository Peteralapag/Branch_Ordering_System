...
$show_limit = isset($_SESSION['BRANCH_MYORDER_LIMIT']) ? $_SESSION['BRANCH_MYORDER_LIMIT'] : 25;
if(isset($_SESSION['BRANCH_MYORDER_BRANCH']))
{
	$branch = $_SESSION['BRANCH_MYORDER_BRANCH'];
} else {
	$branch = $_SESSION['branch_branch'];
}
$userlevel = $_SESSION['branch_userlevel'];

if(isset($_SESSION['BRANCH_MYORDER_RECIPIENT']))
{
	$recipient = $_SESSION['BRANCH_MYORDER_RECIPIENT'];
} else {
	$recipient = '';
}
if(isset($_SESSION['BRANCH_MYORDER_STATUS']))
{
	$order_status = $_SESSION['BRANCH_MYORDER_STATUS'];
} else {
	$order_status = '';
}
?>
...
<div class="pages-wrapper">
    <div class="pages-header">
    	<div class="select-control">
			...
			<span style="position:relative">
				<select id="status" class="form-control form-control-sm" style="width:150px" onchange="load_data()">
					<option value="">--- All Status ---</option>
					<?php echo $function->GetOrderStatusData($order_status, $db); ?>
	    		</select>
...
function performSearch()
{
    ...
    $.post("./Modules/" + module + "/includes/my_orders_data.php", 
        { search: search, branch: branch, limit: limit, recipient: recipient,orderstatus: orderstatus }, 
        function(data) {
            $('#pagesdata').html(data);
            rms_reloaderOff();
        }
...
{	
	...
	document.querySelectorAll('.pagination-btn').forEach(button => {
	    button.addEventListener('click', function () {
	        ...
	        $.post('./Modules/' + module + '/includes/my_orders_data.php', { limit: limit, branch: branch, page: page, recipient: recipient, orderstatus: orderstatus }, function (data) {
	            $('#pagesdata').html(data);
	        });
	    });
	})
...
function loadPage(page)
{
    ...
    $.post('./Modules/' + module + '/includes/my_orders_data.php', { page: page, branch: branch, limit: limit, recipient: recipient, orderstatus: orderstatus }, function(data) {	     
        $('#pagesdata').html(data);       
    });
}
function load_data()
{
	...
	var page = 1;
	rms_reloaderOn('Loading...');
	$.post("./Modules/" + module + "/includes/my_orders_data.php", { page: page, branch: branch, limit: limit, recipient: recipient, orderstatus: orderstatus },
	function(data) {
		$('#pagesdata').html(data);
		rms_reloaderOff();
	});
